...
describe("wooden toad feedback", () => {
  ...
  it("gives each strength a gentle, non-painful reaction", () => {
    expect(woodenToadReaction(0.2)).toMatchObject({ mood: "gentle", label: "眯眼点头" });
    expect(woodenToadReaction(0.6)).toMatchObject({ mood: "steady", label: "鼓腮回应" });
    expect(woodenToadReaction(0.9)).toMatchObject({ mood: "lively", label: "惊喜一跃" });
